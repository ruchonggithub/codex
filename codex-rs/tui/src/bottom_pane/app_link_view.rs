...
impl AppLinkViewParams {
    ...
    fn from_codex_apps_auth_url_parts(
        ...
        Some(Self {
            ...
            title,
            description: None,
            instructions: "请在浏览器中登录此应用，然后返回这里。".to_string(),
            url: url.to_string(),
            is_installed: true,
            is_enabled: true,
    ...
    fn from_generic_url_parts(
        ...
        Self {
            ...
            instructions: "请在浏览器中完成所需操作，然后返回这里。".to_string(),
            url: url.to_string(),
            is_installed: true,
            is_enabled: true,
            suggest_reason: Some(message.to_string()),
...
impl AppLinkView {
    ...
    fn install_confirmation_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        if is_auth_suggestion {
            for line in wrap(
                if is_codex_apps_auth {
                    "请在刚刚打开的浏览器窗口中，通过 ChatGPT 登录该应用。"
                } else {
                    "请在刚刚打开的浏览器窗口中完成身份验证。"
                },
                usable_width,
            ...
            for line in wrap("请在刚刚打开的浏览器窗口中完成所需操作。", usable_width)
            {
                lines.push(Line::from(line.into_owned()));
            }
            ...
            for line in wrap(
                "请在刚刚打开的浏览器窗口中，通过 ChatGPT 完成应用设置。",
                usable_width,
            ) {
                lines.push(Line::from(line.into_owned()));
            }
