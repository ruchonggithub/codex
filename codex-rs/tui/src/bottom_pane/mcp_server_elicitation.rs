...
const ANSWER_PLACEHOLDER: &str = "请输入回答";
const OPTIONAL_ANSWER_PLACEHOLDER: &str = "请输入回答（可选）";
...
impl McpServerElicitationOverlay {
    ...
    fn footer_tips(&self) -> Vec<FooterTip> {
        ...
        if self.current_field_is_select() {
            if self.field_count() == 1 {
                tips.push(FooterTip::highlighted("按 Enter 提交"));
            } else if is_last_field {
                tips.push(FooterTip::highlighted("按 Enter 全部提交"));
            } else {
                tips.push(FooterTip::new("按 Enter 提交回答"));
            }
        ...
        if self.field_count() > 1 {
            if self.current_field_is_select() {
                tips.push(FooterTip::new("按 ←/→ 切换字段"));
            } else {
                tips.push(FooterTip::new("按 Ctrl+P / Ctrl+N 切换字段"));
            }
        }
        tips.push(FooterTip::new("按 Esc 取消"));
        tips
    }
    ...
    fn render_input(&self, area: Rect, buf: &mut Buffer) {
        ...
        if self.current_field_is_select() {
            ...
            render_rows(area, buf, &rows, &state, rows.len().max(1), "没有可用选项");
            return;
        }
...
impl Renderable for McpServerElicitationOverlay {
    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        let progress_line = if self.field_count() > 0 {
            ...
            if unanswered > 0 {
                Line::from(format!("{base}（还有 {unanswered} 个必填项未回答）").dim())
            } else {
                Line::from(base.dim())
            }
        } else {
