...
impl ChatWidget {
    ...
    pub(crate) fn add_connectors_output(&mut self) {
        if !self.connectors_enabled() {
            self.add_info_message(
                "应用功能已禁用。".to_string(),
                Some("请启用应用功能以使用 $ 或 /apps。".to_string()),
            );
            return;
        }
        ...
        match connectors_cache {
            ConnectorsCacheState::Ready(snapshot) => {
                if snapshot.connectors.is_empty() {
                    self.add_info_message("没有可用的应用。".to_string(), /*hint*/ None);
                } else {
    ...
    fn connectors_loading_popup_params(&self) -> SelectionViewParams {
        ...
        SelectionViewParams {
            ...
            items: vec![SelectionItem {
                ...
                description: Some("完整列表准备好后会自动更新。".to_string()),
                is_disabled: true,
                ..Default::default()
            }],
            ..Default::default()
    ...
    fn connectors_popup_params(
        ...
        let mut header = ColumnRenderable::new();
        header.push(Line::from("应用".bold()));
        header.push(Line::from("使用 $ 可将已安装的应用插入提示词。".dim()));
        ...
        for connector in connectors {
            ...
            let selected_label = if is_installed {
                format!("{status_label}。按 Enter 打开应用页面，可安装、管理或启用/禁用此应用。")
            } else {
                format!("{status_label}。按 Enter 打开应用页面并安装此应用。")
            };
        ...
        SelectionViewParams {
            ...
            items,
            is_searchable: true,
            search_placeholder: Some("输入内容以搜索应用".to_string()),
