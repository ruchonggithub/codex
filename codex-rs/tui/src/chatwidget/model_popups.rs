...
impl ChatWidget {
    ...
    pub(crate) fn open_model_popup_with_presets(&mut self, presets: Vec<ModelPreset>) {
        ...
        let header = self.model_menu_header("选择模型", "选择快速自动模式，或浏览所有模型。");
    ...
    pub(crate) fn open_all_models_popup(&mut self, presets: Vec<ModelPreset>) {
        ...
        let header = self.model_menu_header(
            "选择模型和推理强度",
            "如需使用旧版模型，请运行 codex -m <模型名称> 或在 config.toml 中配置",
        );
        self.bottom_pane.show_selection_view(SelectionViewParams {
    ...
    pub(crate) fn open_plan_reasoning_scope_prompt(
        ...
        } else if let Some(plan_mask) = collaboration_modes::plan_mask(self.model_catalog.as_ref())
        {
                ...
                .and_then(|effort| effort.as_ref())
            {
                ...
                None => "内置计划模式默认值（不进行推理）".to_string(),
            }
        } else {
            "内置计划模式默认值".to_string()
        };
        let all_modes_description = format!(
            "设置全局默认推理等级和计划模式覆盖值。这将替换当前的{plan_reasoning_source}。"
        );
        let subtitle = format!("选择将{reasoning_phrase}应用到何处。");
        let warning = effort
            .as_ref()
            .and_then(|effort| self.ultra_reasoning_concurrency_warning(effort));
    ...
    pub(crate) fn open_reasoning_popup(&mut self, preset: ModelPreset) {
        ...
        let warning_text = warn_effort.as_ref().map(|effort| {
            ...
            format!("⚠ {effort_label}推理强度可能会快速消耗 Plus 套餐用量限制。")
        });
    ...
    pub(crate) fn open_advanced_reasoning_popup(&mut self, preset: ModelPreset) {
        ...
        for effort in choices {
            let description = match &effort {
                ReasoningEffortConfig::Max => "适用于质量重于速度的复杂问题 · 用量更高",
                ReasoningEffortConfig::Ultra => "适用于使用多个代理的高要求任务 · 用量最高",
        ...
        header.push(Line::from("⚠ 会更快消耗用量限制".cyan()));
        self.bottom_pane.show_selection_view(SelectionViewParams {
            header: Box::new(header),
            footer_hint: Some(standard_popup_hint_line()),
            items,
    ...
    pub(super) fn ultra_reasoning_concurrency_warning(
        ...
        Some(format!(
            "极致推理可能会主动使用多个代理。此会话配置为最多 {max_threads} 个并发线程，\
             其中最多 {max_subagents} 个子代理，可能快速增加用量。建议将 \
             features.multi_agent_v2.max_concurrent_threads_per_session 设置为小于 8。"
        ))
    }
