...
impl ChatWidget {
    pub(crate) fn add_plugins_output(&mut self) {
        if !self.config.features.enabled(Feature::Plugins) {
            self.add_info_message(
                "插件功能已禁用。".to_string(),
                Some("请启用插件功能以使用 /plugins。".to_string()),
            );
            return;
        }
            ...
            Ok(response) => {
                ...
                if selected_count == 0 {
                    self.add_info_message(
                        ...
                        Some("只能升级已配置的 Git 市场。".to_string()),
                    );
                    return;
                }
    ...
    pub(crate) fn on_plugin_enabled_set(
        ...
        if let Err(err) = result {
            self.add_error_message(format!("无法更新插件 {plugin_id} 的配置：{err}"));
    ...
    fn plugin_install_auth_popup_params(&self) -> Option<SelectionViewParams> {
        ...
        if let Some(install_url) = app.install_url.clone() {
            ...
            items.push(SelectionItem {
                ...
                description: Some("此应用未提供安装/管理地址。".to_string()),
                is_disabled: true,
                ..Default::default()
            });
        }
        ...
        if is_installed {
            items.push(SelectionItem {
                ...
                selected_description: Some("继续设置下一个应用。".to_string()),
            ...
            items.push(SelectionItem {
                name: "我已安装".to_string(),
                description: Some("采用你的确认并继续设置下一个应用。".to_string()),
        ...
        items.push(SelectionItem {
            ...
            description: Some("停止此插件的后续设置流程。".to_string()),
            selected_description: Some("放弃设置剩余的必需应用。".to_string()),
            actions: vec![Box::new(|tx| {
    ...
    fn finish_plugin_install_auth_flow(&mut self, abandoned: bool) {
        ...
        if abandoned {
            self.add_info_message(
                format!("已跳过 {} 插件的其余应用设置。", flow.plugin_display_name),
                Some("安装所需应用前，该插件可能无法使用。".to_string()),
            );
        } else {
            self.add_info_message(
                format!("已完成 {} 插件的应用设置流程。", flow.plugin_display_name),
                Some("现在可以通过 /plugins 继续管理插件。".to_string()),
            );
        }
