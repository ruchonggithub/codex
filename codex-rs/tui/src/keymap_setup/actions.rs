...
pub(super) const KEYMAP_ACTIONS: &[KeymapActionDescriptor] = &[
    ...
    action("global", "全局", "copy", "将代理的上一条回复复制到剪贴板。"),
    action("global", "全局", "clear_terminal", "清空终端界面。"),
    action("global", "全局", "toggle_vim_mode", "开启或关闭输入框的 Vim 模式。"),
    gated_action("global", "全局", "toggle_fast_mode", "开启或关闭快速模式。", KeymapActionFeature::FastMode),
    action("global", "全局", "toggle_raw_output", "切换原始回滚输出模式。"),
    action("chat", "聊天", "interrupt_turn", "中断当前正在执行的任务。"),
    action("chat", "聊天", "decrease_reasoning_effort", "降低推理强度。"),
    action("chat", "聊天", "increase_reasoning_effort", "提高推理强度。"),
    action("chat", "聊天", "edit_queued_message", "编辑最近排队的消息。"),
    action("composer", "输入框", "submit", "提交当前输入框草稿。"),
    action("composer", "输入框", "queue", "任务运行时将草稿加入队列。"),
    action("composer", "输入框", "toggle_shortcuts", "显示或隐藏输入框快捷键浮层。"),
    action("composer", "输入框", "history_search_previous", "打开历史搜索或移到上一个匹配项。"),
    action("composer", "输入框", "history_search_next", "移到下一个历史搜索匹配项。"),
    action("editor", "编辑器", "insert_newline", "在编辑器中插入换行。"),
    action("editor", "编辑器", "move_left", "将光标向左移动。"),
    action("editor", "编辑器", "move_right", "将光标向右移动。"),
    action("editor", "编辑器", "move_up", "将光标向上移动。"),
    action("editor", "编辑器", "move_down", "将光标向下移动。"),
    action("editor", "编辑器", "move_word_left", "移到上一个单词的开头。"),
    action("editor", "编辑器", "move_word_right", "移到下一个单词的末尾。"),
    action("editor", "编辑器", "move_line_start", "移到行首。"),
    action("editor", "编辑器", "move_line_end", "移到行尾。"),
    action("editor", "编辑器", "delete_backward", "删除左侧的一个字素。"),
    action("editor", "编辑器", "delete_forward", "删除右侧的一个字素。"),
    action("editor", "编辑器", "delete_backward_word", "删除前一个单词。"),
    action("editor", "编辑器", "delete_forward_word", "删除后一个单词。"),
    action("editor", "编辑器", "kill_line_start", "删除光标到行首的内容。"),
    action("editor", "编辑器", "kill_whole_line", "删除当前整行。"),
    action("editor", "编辑器", "kill_line_end", "删除光标到行尾的内容。"),
    action("editor", "编辑器", "yank", "粘贴删除缓冲区中的内容。"),
    ...
    action("vim_normal", "Vim 普通模式", "move_left", "在 Vim 普通模式中向左移动。"),
    action("vim_normal", "Vim 普通模式", "move_right", "在 Vim 普通模式中向右移动。"),
    action("vim_normal", "Vim 普通模式", "move_up", "向上移动，或调出更早的历史记录。"),
    action("vim_normal", "Vim 普通模式", "move_down", "向下移动，或调出更新的历史记录。"),
    action("vim_normal", "Vim 普通模式", "move_word_forward", "移到下一个单词的开头。"),
    action("vim_normal", "Vim 普通模式", "move_word_backward", "移到上一个单词的开头。"),
    action("vim_normal", "Vim 普通模式", "move_word_end", "移到当前或下一个单词的末尾。"),
    action("vim_normal", "Vim 普通模式", "move_line_start", "移到行首。"),
    action("vim_normal", "Vim 普通模式", "move_line_end", "移到行尾。"),
    action("vim_normal", "Vim 普通模式", "delete_char", "删除光标下的字符。"),
    action("vim_normal", "Vim 普通模式", "substitute_char", "删除光标下的字符并进入插入模式。"),
    action("vim_normal", "Vim 普通模式", "delete_to_line_end", "删除光标到行尾的内容。"),
    action("vim_normal", "Vim 普通模式", "change_to_line_end", "修改光标到行尾的内容并进入插入模式。"),
    action("vim_normal", "Vim 普通模式", "yank_line", "复制当前整行。"),
    action("vim_normal", "Vim 普通模式", "paste_after", "在光标后粘贴。"),
    action("vim_normal", "Vim 普通模式", "start_delete_operator", "开始删除操作并等待移动指令。"),
    action("vim_normal", "Vim 普通模式", "start_yank_operator", "开始复制操作并等待移动指令。"),
    action("vim_normal", "Vim 普通模式", "start_change_operator", "开始修改操作并等待文本对象。"),
    action("vim_normal", "Vim 普通模式", "cancel_operator", "取消待处理的 Vim 操作符。"),
    action("vim_operator", "Vim 操作符", "delete_line", "重复删除操作符以删除整行。"),
    action("vim_operator", "Vim 操作符", "yank_line", "重复复制操作符以复制整行。"),
    action("vim_operator", "Vim 操作符", "motion_left", "将操作符向左移动。"),
    action("vim_operator", "Vim 操作符", "motion_right", "将操作符向右移动。"),
    action("vim_operator", "Vim 操作符", "motion_up", "将操作符向上移动。"),
    action("vim_operator", "Vim 操作符", "motion_down", "将操作符向下移动。"),
    action("vim_operator", "Vim 操作符", "motion_word_forward", "将操作符移到下一个单词开头。"),
    action("vim_operator", "Vim 操作符", "motion_word_backward", "将操作符移到上一个单词开头。"),
    action("vim_operator", "Vim 操作符", "motion_word_end", "将操作符移到单词末尾。"),
    action("vim_operator", "Vim 操作符", "motion_line_start", "将操作符移到行首。"),
    action("vim_operator", "Vim 操作符", "motion_line_end", "将操作符移到行尾。"),
    action("vim_operator", "Vim 操作符", "select_inner_text_object", "选择内部文本对象。"),
    action("vim_operator", "Vim 操作符", "select_around_text_object", "选择包含周边的文本对象。"),
    action("vim_operator", "Vim 操作符", "cancel", "取消待处理的操作符。"),
    action("vim_text_object", "Vim 文本对象", "word", "选中当前单词。"),
    action("vim_text_object", "Vim 文本对象", "big_word", "选中当前 WORD。"),
    action("vim_text_object", "Vim 文本对象", "parentheses", "选中外层圆括号。"),
    action("vim_text_object", "Vim 文本对象", "brackets", "选中外层方括号。"),
    action("vim_text_object", "Vim 文本对象", "braces", "选中外层花括号。"),
    action("vim_text_object", "Vim 文本对象", "double_quote", "选中外层双引号。"),
    action("vim_text_object", "Vim 文本对象", "single_quote", "选中外层单引号。"),
    action("vim_text_object", "Vim 文本对象", "backtick", "选中外层反引号。"),
    action("vim_text_object", "Vim 文本对象", "cancel", "取消待处理的文本对象。"),
    ...
    action("pager", "分页器", "jump_top", "跳到开头。"),
    action("pager", "分页器", "jump_bottom", "跳到末尾。"),
    action("pager", "分页器", "close", "关闭分页器浮层。"),
    action("pager", "分页器", "close_transcript", "关闭对话记录浮层。"),
    action("list", "列表", "move_up", "将列表选项向上移动。"),
    action("list", "列表", "move_down", "将列表选项向下移动。"),
    action("list", "列表", "move_left", "在列表选择器中向左移动。"),
    action("list", "列表", "move_right", "在列表选择器中向右移动。"),
    action("list", "列表", "page_up", "将列表选项向上移动一页。"),
    action("list", "列表", "page_down", "将列表选项向下移动一页。"),
    action("list", "列表", "jump_top", "跳到列表第一项。"),
    action("list", "列表", "jump_bottom", "跳到列表最后一项。"),
    action("list", "列表", "accept", "接受当前列表选项。"),
    action("list", "列表", "cancel", "取消并关闭选择视图。"),
    action("approval", "审批", "open_fullscreen", "全屏打开审批详情。"),
    action("approval", "审批", "open_thread", "在可用时打开审批来源线程。"),
    action("approval", "审批", "approve", "批准主要选项。"),
    action("approval", "审批", "approve_for_session", "在可用时批准本次会话。"),
    action("approval", "审批", "approve_for_prefix", "在可用时按执行策略前缀批准。"),
    action("approval", "审批", "deny", "在可用时选择明确拒绝。"),
    action("approval", "审批", "decline", "拒绝并提供纠正说明。"),
...
pub(super) fn action_label(action: &str) -> String {
    match action {
        ...
        "move_up" => "向上移动",
        "move_down" => "向下移动",
        "move_word_left" => "向左移动一个单词",
        "move_word_right" => "向右移动一个单词",
        "move_line_start" => "移到行首",
        "move_line_end" => "移到行尾",
        "delete_backward" => "向后删除",
        "delete_forward" => "向前删除",
        "delete_backward_word" => "删除前一个单词",
        "delete_forward_word" => "删除后一个单词",
        "kill_line_start" => "删除到行首",
        ...
        "move_word_forward" => "移到下一个单词",
        "move_word_backward" => "移到上一个单词",
        "move_word_end" => "移到单词末尾",
        "delete_char" => "删除字符",
        "substitute_char" => "替换字符",
        "delete_to_line_end" => "删除到行尾",
        "change_to_line_end" => "修改到行尾",
        "yank_line" => "复制整行",
        "paste_after" => "在光标后粘贴",
        ...
        "delete_line" => "删除整行",
        "motion_left" => "操作符向左移动",
        "motion_right" => "操作符向右移动",
        "motion_up" => "操作符向上移动",
        "motion_down" => "操作符向下移动",
        "motion_word_forward" => "操作符移到下一个单词",
        "motion_word_backward" => "操作符移到上一个单词",
        "motion_word_end" => "操作符移到单词末尾",
        "motion_line_start" => "操作符移到行首",
        "motion_line_end" => "操作符移到行尾",
        "select_inner_text_object" => "选择内部文本对象",
        "select_around_text_object" => "选择周围文本对象",
        "word" => "当前单词",
        "big_word" => "当前 WORD",
        "parentheses" => "圆括号",
        "brackets" => "方括号",
        ...
        "open_thread" => "打开来源线程",
        "approve" => "批准",
        "approve_for_session" => "本会话内批准",
        "approve_for_prefix" => "按命令前缀批准",
        "deny" => "拒绝",
        "decline" => "拒绝并提供说明",
        "cancel" => "取消",
        _ => "其他操作",
...
pub(super) fn action_description(_action: &str, source_description: &str) -> String {
        ...
        .filter_map(|descriptor| {
            ...
            bindings
                ...
                .then(|| KeymapDebugActionMatch {
                    ...
                    description: action_description(descriptor.action, descriptor.description),
