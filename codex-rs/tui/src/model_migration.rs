...
pub(crate) fn migration_copy_for_models(
    ...
    let heading_text = Span::from(format!("Codex 已升级，现推出 {target_display_name}。")).bold();
    ...
    if let Some(migration_copy) = &migration_copy {
        ...
        description_line = target_description
            ...
            .unwrap_or_else(|| {
                Line::from(format!(
                    "推荐使用 {target_display_name}，以获得更好的性能和可靠性。"
                ))
            });
    }
    ...
    if let Some(model_link) = model_link {
        content.push(Line::from(vec![
            format!("{description_line} 了解有关 {target_display_name} 的更多信息：").into(),
...
impl ModelMigrationScreen {
    ...
    fn render_menu(&self, column: &mut ColumnRenderable) {
        column.push(Line::from(""));
        column.push(
            Paragraph::new("选择 Codex 接下来的处理方式。")
                .wrap(Wrap { trim: false })
                .inset(Insets::tlbr(
                    /*top*/ 0, /*left*/ 2, /*bottom*/ 0, /*right*/ 0,
                )),
