...
impl SlashCommand {
    ...
    pub fn description(self) -> &'static str {
        match self {
            ...
            SlashCommand::Compact => "总结对话，避免达到上下文上限",
            ...
            SlashCommand::Ide => "添加 IDE 当前选区、打开的文件及其他上下文",
            SlashCommand::Personality => "选择 Codex 的沟通风格",
            SlashCommand::Plan => "切换到计划模式",
            SlashCommand::Goal => "设置或查看长时间任务的目标",
            SlashCommand::Agent | SlashCommand::MultiAgents => "切换当前代理线程",
            SlashCommand::Side | SlashCommand::Btw => "在临时分支中开始旁路对话",
            SlashCommand::Permissions => "选择允许 Codex 执行的操作",
            SlashCommand::Keymap => "重新映射 TUI 快捷键",
            SlashCommand::Vim => "开关输入框的 Vim 模式",
            SlashCommand::ElevateSandbox => "设置高权限代理沙箱",
            SlashCommand::SandboxReadRoot => "允许沙箱读取目录：/sandbox-add-read-dir <绝对路径>",
            SlashCommand::Experimental => "开关实验性功能",
            SlashCommand::AutoReview => "批准重试最近一次被自动审查拒绝的操作",
